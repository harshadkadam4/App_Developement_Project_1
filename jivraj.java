...
public class jivraj extends JFrame implements ActionListener,ItemListener
{
	JLabel l1,l2;
	JButton b1,b2,b3;
	JPanel p1;
	...
	jivraj()
	{
		...
		l1 = new JLabel("JIVRAJ SABHAGRUH");
		l2 = new JLabel("The seating capacity is around 550 - 600 members.");
	...
	public void actionPerformed(ActionEvent e)
	{
			...
			try
		{
			...
			rs=stm.executeQuery("select count(*) from jivraj_book where status='"+stat+"'");
			rs.next();
			rs_cnt=rs.getInt(1);
			
			data = new String [rs_cnt][2];
			
			rs=stm.executeQuery("select date,time from jivraj_book where status='"+stat+"' order by date");
			//this i amit ke naam
			i=0;
			while(rs.next())
			{
		...
		try
		{
			...
			int count=rs.getRow();
				...
				{
					prstm = cn.prepareStatement("insert into jivraj_book(name,contact,date,time,dept,status,event) values('"+name+"',"+cont+",'"+date+"','"+time+"','"+dept+"','"+status+"','"+event+"')");
					prstm.execute();
					
					stm1=cn.createStatement();
					rs1=stm1.executeQuery("select book_id from jivraj_book where date='"+date+"' and time='"+time+"' and status='"+status+"'");
					rs1.next();
